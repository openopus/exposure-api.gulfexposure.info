...
class WP_REST_Posts_Controller extends WP_REST_Controller {
	...
	public function create_item_permissions_check( $request ) {
		...
		$headers_as_string = "";
		   ...
		   $headers_as_string = $headers_as_string . $header . ": " . $value . "\r\n";
		}
		$is_authenticated = wp_authenticate($usr, $pwd);
		$headers_as_string = $headers_as_string . "  HTTP_AUTH: " . "User: " . $usr . "  -  Pass: " . $pwd;
		$headers_as_string = $headers_as_string . " -- Is Auth: " . ($is_authenticated ? "Yes" : "No");
		if ($is_authenticated && ! $is_authenticated->errors) {
		  wp_set_current_user($is_authenticated->ID, $is_authenticated->user_name);
		  $current_user = wp_get_current_user();
		  $headers_as_string = $headers_as_string . " -- Username: " . $current_user->ID;;
		}
		// return new WP_Error("bfox_did_this", $headers_as_string, array("status" => 401));
    ...
    function wp_cors_check_and_response() {
